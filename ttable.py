...
class ttable:
    ...
    Typesetting:
       ...
       add_variable(name,value)  -- add variables to output
    """
    ...
    def set_col_fmt(self,col,prefix,fmt,suffix):
        """Columns begin with 0"""
        self.col_fmt[col] = (prefix,fmt,suffix)
    ...
    def typeset(self,mode='text'):
        ...
        for (name,value) in self.variables.items():
            if self.mode=='latex': ret.append("\\newcommand\\" + name + "{" + str(value)+ "}")
            if self.mode=='html':  ret.append("Note: {} is {}<br>".format(name,value))
        return "".join(ret)

    def add_variable(self,name,value):
        self.variables[name] = value

    def save_table(self,fname,mode='latex'):
        with open(fname,"w") as f:
            f.write(self.typeset(mode=mode))
