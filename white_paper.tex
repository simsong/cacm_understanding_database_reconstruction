...
solvers, and many of them take input in the so-called DIMACS file
format, which is a standardized form for representing CNF problems.
...
clauses in just a few minutes. In our case, the open source
...
program is not limited by RAM).

\subsection{Exploring the Solution Universe}

PicoSAT's solution is a satisfying assignment for the \NumVariables{}
Boolean variables. After PicoSAT runs, we can use Sugar to translate
...
problem. Many of these solutions are degenerate, in that they are
different assignments of the internal Boolean variables that result in
the same higher-level variables.  In the case of this problem, there
are actually 3,554 degenerate satisfying solutions, each of which maps
to the same distinct assignment of attributes to the microdata that produce the statistics in
...
\end{minipage}
\end{center}  
Table~\ref{fictional} is actually over-constrained: some (but not all) of the
constraints can be dropped while preserving a unique
solution.

However, rather than looking for the ways that the table is
over-constrained, it is more useful here to explore what happens when
a critical statistic is suppressed. After all, statistical agencies
have long used suppression (censoring) in an attempt to provide
privacy to those whose attributes are presented in the microdata. How
...
degenerate solutions from 3,554 to 12,498. More importantly, it
increases the number of distinct solutions from 1 to 3. The three
satisfying solutions are:

\begin{center}
\begin{tabular}{c|c|c}
  Solution \#1 & Solution \#2 & Solution \#3\\
\hline
 \texttt{ 8FBS } &  \texttt{ 8FBS} & \texttt{ 8MBS} \\
 \texttt{18MWS } &  \texttt{18FWS} & \texttt{18FWS} \\
 \texttt{24FWS } &  \texttt{24MWS} & \texttt{24FWS} \\
 \texttt{30MWM } &  \texttt{30MWM} & \texttt{30MWM} \\
 \texttt{36FBM } &  \texttt{42FBM} & \texttt{36FBM} \\
 \texttt{66FBM } &  \texttt{66FBM} & \texttt{56FBM} \\
 \texttt{84MBM } &  \texttt{78MBM} & \texttt{94MBM} \\
  \end{tabular}
\end{center}

Notice that each of these solutions contains 30MWM (age 30, Male,
White, Married). Since a record with these attributes is common to all
of the solutions, this record is revealed even when statistic 4A is
suppressed.  For the remaining persons, it is not clear how to report
the relative odds that an individual matching those characteristics is
present. There are vastly more degenerate solutions pointing to
Solution \#1 than to Solution \#2 or \#3, and Solution \#1 is the
solution that PicoSAT reports first in almost all cases. However, all three solutions
are allowed by the constraints, so they are all theoretically
possible. 
