...
specific attributes for particular persons or establishments.
Traditionally, these privacy guarantees are assured through the use of
\emph{Statistical Disclosure Limitation} (SDL) techniques. These
techniques are not sufficient to prevent a database reconstruction
attack of the sort anticipated by Dinur and Nissim (2003). To
illustrate the problem, this paper presents a database reconstruction
attack on a hypothetical block for which statistics have been
published by an official statistics agency. We then show how the use
of noise introduced to satisfy the requirements of a formal privacy
model reduces the privacy risk of a database reconstruction. The
paper concludes with a discussion of the implications for the 2020
...
be generalized to many more settings.
