...
s-expression:
\begin{verbatim}
(= (+ A1 A2 A3 A4 A5 A6 A7 A8 A9 A10) 10) 400)
\end{verbatim}

Notice that we do not encode the division operator. Because SAT
solvers can only perform operations on Boolean values, the division
operator would not yield exact solutions due to integer round-off error.

Our source input file is \NumConstraintLines{} (minus comments)
consisting of \NumSExpressions s-expressions. This converts into
