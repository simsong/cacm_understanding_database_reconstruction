...
\begin{abstract}
In recent years, a certain type of database attack,
...
In this section we examine the most relevant literature regarding the DRA. Although this attack is relatively new due to the aforementioned recent development of attack technology, several groups have performed research on important concepts used in the DRA such as database inference from statistics and query overlap.
...
\textit{Please provide the following for each member
of your family:
...
The data in Table~\ref{resultsencodedsmall} is the 'ground truth' database that the attacker wishes to reconstruct. At the start of the attack, the attacker writes the table of 25 unknowns given in Table~\ref{unknownssmall}.
...
\textit{Individuals: Number = 5, Average Age = 24}
...
\textit{Number of Grandparents: 1}
...
\textit{Children: Number = 2, Average Age = 6.5}
...
The ground truth responses from two new households are given in Table~\ref{resultsbig}.
\begin{table}
\begin{tabular}{c|c|c|c|c|c}
ID & Household & Age & Sex & Race & Generation \\
\hline
...
The statistical agency publishes the statistics in Table~\ref{publishedstatsbig} after processing the ground truth responses.
...
Output is given in Table~\ref{sugarbig} and is tabulated for readability. Although the ID numbers are different, the ten people in the SAT solver output are identical to the ten people from the ground truth in Table~\ref{resultsbig}, so the attack has succeeded in reconstructing the entire database and narrowing down the solution universe to size 1 in only 35.51 seconds.
...
\subsection{Scalability and The Zebra Problem}

To demonstrate the real capabilities of the SAT solver when faced with a very large constraint system, in contrast to our above relatively small examples, we will use a system with 155 variables and 1135 Boolean clauses. This system results from the CNF encoding of a famous problem known as the Zebra Problem \cite{zebra}.
...
Once the attacker has generated every constraint possible from the given information, he can input the constraints into a SAT solver. In this example, we run the zebra problem's CNF encoding through PycoSAT, a free Python wrapper of PicoSAT.

The program identifies that the variables "Norwegian drinks water" and the "Japanese owns the zebra" are true in only 0.00046s.
...
Positive integers correspond to True, while negative integers are False. For example, $118$ in the solution output means that the Boolean variable "The Spaniard drinks orange juice" is True. The full set of variable encodings is given in Appendix 5.
...
;
;avg age total
(= (/ (+ A1 A2 A3 A4 A5 A6 A7 A8 A9 A10) 10) 40)
;household sizes
...
\subsection{Zebra Problem Variable Encodings}

Below are the variable encodings in C for the zebra problem. Each variable is a boolean represented in the output by a negative number if it is false and a positive number if it is true.
